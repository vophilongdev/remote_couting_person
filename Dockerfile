...
RUN pip install --no-cache-dir /app_root/app/services/sdk/dist/NetSDK-2.0.0.1-py3-none-linux_x86_64.whl

# Copy legacy OpenSSL 1.1 libraries to system path for Dahua NetSDK dependency
RUN cp /app_root/app/services/sdk/dist/libcrypto.so.1.1 /usr/lib/ && \
    cp /app_root/app/services/sdk/dist/libssl.so.1.1 /usr/lib/
...
RUN for dir in /usr/local/lib/python3.10/site-packages/NetSDK/Libs/linux64 /usr/local/lib/python3.10/dist-packages/NetSDK/Libs/linux64; do \
    ...
    echo "/app_root/app/services/sdk/dist" >> /etc/ld.so.conf.d/netsdk.conf && \
    ldconfig
...
# Run FastAPI Gateway Uvicorn Server (using new module path)
