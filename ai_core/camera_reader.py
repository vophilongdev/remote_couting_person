# -*- coding: utf-8 -*-
"""
Multi-Source High-Performance Camera Reader
...
class CameraReader:
    ...
    def _read_opencv(self):
        ...
        if not cap.isOpened():
            logger.error(f"Failed to open OpenCV VideoCapture source: {self.source}")
            self.is_connected = False
            return
        ...
        while not self.shutdown_flag.is_set():
            ...
            if not ret or frame is None:
                if isinstance(src, str) and not src.isdigit():
                    ...
                    logger.warning("Camera stream disconnected. Re-connecting in 1 second...")
                    time.sleep(1.0)
                    cap.release()
                    cap = cv2.VideoCapture(src)
                    cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
